...
(desktop); use native video fullscreen on touch devices (web).
...
`allowPrerelease` from the app's own version, and the `-beta` suffix upstream
carries turns it on, which makes it skip every release whose tag is not valid
semver — ours are all `wc-photos-desktop-v*`. A build left at upstream's number
can never update itself, and cannot identify itself in `ente.log` either.
...
Thirteen inherited workflows (Crowdin sync, Cloudflare deploys, the mobile /
